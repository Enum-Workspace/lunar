...
import { createManualIntervalClock, } from "../time/clock.js";
import { BatchBuffer } from "./batch-buffer.js";
function makeBuffer(params) {
...
describe("BatchBuffer", () => {
    describe("add()", () => {
        it("accumulates items without flushing when under maxBufferSize", async () => {
            const flushed = [];
            const { buf, clock: _clock } = makeBuffer({
                maxBufferSize: 5,
                onFlush: (items) => {
                    flushed.push([...items]);
                },
...
//# sourceMappingURL=batch-buffer.test.js.map