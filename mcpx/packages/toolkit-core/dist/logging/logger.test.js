...
import { buildLogger, DEFAULT_REDACT_KEYS, redactObject, redactUrl, } from "./logger.js";
describe("redactObject", () => {
    ...
    it("preserves class instances", () => {
        ...
        expect(result["instance"].getValue()).toBe("test"); // Casting to MyClass to access method, if this wasn't preserved correctly, this would fail
        expect(result["env"]).toBe("[REDACTED]");
    });
...
describe("redactUrl", () => {
    ...
    it("redacts tokens and keys in the query, keeping the rest", () => {
        expect(redactUrl("/cb?access_token=abc123&page=2")).toBe("/cb?access_token=%5BREDACTED%5D&page=2");
    });
    it("redacts the OAuth code param", () => {
        expect(redactUrl("/oauth/callback?code=secret&state=xyz")).toBe("/oauth/callback?code=%5BREDACTED%5D&state=xyz");
    });
...
describe("buildLogger redaction (end-to-end)", () => {
    ...
    async function captureOutput(log, redactKeys) {
        const chunks = [];
    ...
    it("redacts default-sensitive keys (incl. nested) without any config", async () => {
        const output = await captureOutput((logger) => logger.info("request done", {
    ...
    it("matches sensitive keys case-insensitively end-to-end", async () => {
        const output = await captureOutput((logger) => logger.info("auth", { Authorization: "Bearer leak-me" }));
        expect(output).not.toContain("leak-me");
        expect(output).toContain("[REDACTED]");
    });
    it("treats caller-supplied redactKeys as additive to the defaults", async () => {
        const output = await captureOutput((logger) => logger.info("custom", {
