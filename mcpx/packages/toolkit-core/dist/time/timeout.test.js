...
describe("withTimeout", () => {
    ...
    describe("timeout behavior", () => {
        ...
        it("includes operation name in error message", async () => {
            const promise = new Promise(() => { });
            await expect(withTimeout(promise, 10, "some-long-running-job")).rejects.toThrow("some-long-running-job timed out after 10ms");
        });
    ...
    describe("error propagation", () => {
        ...
        it("propagates rejection even for fast failures", async () => {
            const promise = new Promise((_, reject) => setTimeout(() => reject(new Error("fast failure")), 5));
