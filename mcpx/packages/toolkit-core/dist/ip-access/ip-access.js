...
function compileOne(rawIP) {
    const s = rawIP.trim();
    const hasSlash = s.includes("/");
    const [ipStr, prefixStr] = hasSlash ? s.split("/", 2) : [s, ""];
    if (!ipStr)
        throw new Error(`Invalid IP in range: ${s}`);
...
export function compileRanges(ranges) {
    return ranges.map(compileOne);
}
function isIPv6(addr) {
    return addr.kind() === "ipv6";
}
export function ipAllowed(ip, compiled) {
    const addr = ipaddr.parse(ip);
    const norm = isIPv6(addr) && addr.isIPv4MappedAddress() ? addr.toIPv4Address() : addr;
    return compiled.some(([base, bits]) => {
        if (norm.kind() !== base.kind())
            return false;
        return norm.match([base, bits]);
    });
}
export function makeIpAllowlistMiddleware(compiled, logger) {
    return (req, res, next) => {
