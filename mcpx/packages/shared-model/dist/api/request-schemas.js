...
export const isValidHeaderTemplateString = (value) => HEADER_VALIDATION_REGEX.test(value);
// ZOD
...
export function isEmptyPrefilled(value) {
...
export const envRequirementSchema = envRequirementBaseSchema.transform((req) => {
    if (req.kind === "fixed")
        return req;
...
export const updateTargetServerStdioRequestSchema = createTargetServerStdioRequestSchema.omit({
    name: true,
});
export const updateTargetServerSSESchema = createTargetServerSSESchema.omit({
    name: true,
});
export const updateTargetServerStreamableHttpSchema = createTargetServerStreamableHttpSchema.omit({
    name: true,
});
...
export const applyParsedAppConfigRequestSchema = z.record(z.string(), z.unknown());
