...
import { authSchema, toolGroupSchema, } from "./config.js";
...
function inferConsumerConfigType(val) {
    if (val && typeof val === "object" && !("_type" in val)) {
        if ("allow" in val)
            return { ...val, _type: "default-block" };
        if ("block" in val)
            return { ...val, _type: "default-allow" };
    }
    return val;
}
...
export const consumerConfigSchema = z.preprocess(inferConsumerConfigType, z.union([defaultAllowConsumerConfig, defaultBlockConsumerConfig]));
...
export function scopeSubjectKey(subject) {
    return `${subject.kind}:${subject.value}`;
}
export function scopeSubjectsEqual(a, b) {
...
export const skillsConfigSchema = z
    ...
    .optional()
    .default({ enabled: [] });
export const toolExtensionParamsSchema = z.lazy(() => z
...
export const toolExtensionsServiceSchema = z.record(z.string(), z.object({
...
const legacyLiteralClientCredentials = z
    ...
    .transform((c) => ({
    clientId: { type: "literal", value: c.clientId },
    clientSecret: { type: "literal", value: c.clientSecret },
}));
const legacyEnvRefClientCredentials = z
    ...
    .transform((c) => ({
    clientId: { type: "envRef", envName: c.clientIdEnv },
    clientSecret: { type: "envRef", envName: c.clientSecretEnv },
}));
